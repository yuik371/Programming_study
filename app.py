from flask import Flask, render_template, request, jsonify, send_from_directory, render_template # Imoporting necessary modules from Flask
...
import matplotlib 
matplotlib.use('Agg') # Setting matplotlib to use the 'Agg' backend for non-GUI enviroments
import matplotlib as plt # For creating plots 
import os
...
# Route for serving the graph page
@app.route('/api/recalls/graph', methods=['GET'])
def graph_page():
  return send_from_directory('static', 'graph.html') # Serves the 'graph.html' from the 'static' directory

# Route for getting recall counts
@app.route('/api/recall_counts', methods=['GET']) 
